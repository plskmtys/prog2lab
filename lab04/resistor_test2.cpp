...
int main() {
  ...
  Resistor r; //default konstruktor

  TEST(Resistor, HazibanAdottR) {
    double d = r.getR();
    EXPECT_EQ(104., d) << "Jporta feladatban megadott default ertekkel hasonlitson!" << std::endl;
  } END

  Resistor r1(1000), r2(3000); //konstruktor double-al
  ...
  TEST(ResistorTest, getI) {				// getI
    const Resistor r0(2); //konstruktor double-al
  ...
  TEST(ResistorTest, getU) {				// getU
    const Resistor r0(12.12);//konstruktor double-al
  ...
  TEST(ResistorTest, EQ) {				    // operator==
    const Resistor r0(12345.6789);//konstruktor double-al
    const Resistor r1(123456789); //konstruktor double-al
    ...
    const Resistor r3(1e-18); //konstruktor double-al
    r2 = r3 + Resistor(1e-18); //konstruktor double-al
    EXPECT_FALSE(r3 == r1);
  } END
#endif
  ...
  TEST(ResistorTest, Inserter) {
    ...
    oss << Resistor(8.45) << '\t' << Resistor(100); //konstruktor double-al
